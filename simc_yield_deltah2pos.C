...
void simc_yield_deltah2pos(){
  ...
  ifstream infile(Form("/lustre24/expphy/volatile/hallc/c-csv/hdbhatt/simcjuly22/outfiles/csv_%d_H2_pos_delta.hist",runs[r]));
...
cout<<normfac_value<<endl;
    ...
    TString filename  = Form("/lustre24/expphy/volatile/hallc/c-csv/hdbhatt/simcjuly22/worksim/csv_%d_H2_pos_delta.root",runs[r]);
...
#include "includeDeltaExc.h"
    ...
    std::string rootfile_yield_name = Form("yield_hists/yield_simc_delta_h2pos_%d.root",runs[r]);
    ...
    c1->SaveAs(Form("plots_simc/plots_h2pos_delta_%d.pdf",runs[r]));
