...
void simc_yield_noradh2pos(){
  ...
  ifstream infile(Form("/lustre24/expphy/volatile/hallc/c-csv/hdbhatt/simcjuly22/outfiles/csv_%d_H2_pos_inc_norad.hist",runs[r]));
...
cout<<normfac_value<<endl;
    ...
    TString filename  = Form("/lustre24/expphy/volatile/hallc/c-csv/hdbhatt/simcjuly22/worksim/csv_%d_H2_pos_inc_norad.root",runs[r]);
...
#include "includeNoRad.h"
    ...
    std::string rootfile_yield_name = Form("yield_hists/yield_simc_inc_norad_h2pos_%d.root",runs[r]);
    ...
    c1->SaveAs(Form("plots_simc/plots_h2pos_inc_norad_%d.pdf",runs[r]));
