...
zhad = Epi/nu;
mx2 = (protonmass + nu - zhad*nu) * (protonmass + nu - zhad*nu) - abs(pm)*abs(pm);
...
//if(acceptance_cut_coin && peter_all_geom_cut && pt< 0.25 && mx2 > 2.56 ){ W2 is added on Oct12, 2023) in data and simc both, 
//if(acceptance_cut_coin && peter_all_geom_cut && pt< 0.25 && mx2 > 2.56 && W2 > 4 ){
if(acceptance_cut_coin && peter_all_geom_cut){

  //note hgcer remove region is not used???
