...
void simc_yield_rhoh2pos(){
  ...
  ifstream infile(Form("/lustre24/expphy/volatile/hallc/c-csv/hdbhatt/simcjuly22/outfiles/csv_%d_H2_pos_rho.hist",runs[r]));
...
cout<<normfac_value<<endl;
    ...
    TString filename  = Form("/lustre24/expphy/volatile/hallc/c-csv/hdbhatt/simcjuly22/worksim/csv_%d_H2_pos_rho.root",runs[r]);
...
#include "includeRad.h"
    ...
    std::string rootfile_yield_name = Form("yield_hists/yield_simc_rho_h2pos_%d.root",runs[r]);
    ...
    c1->SaveAs(Form("plots_simc/plots_h2pos_rho_%d.pdf",runs[r]));
