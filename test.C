...
void test(){
  ...
  for (int r =0;r<numRuns;r++){
   ...
   ifstream infile(Form("/lustre24/expphy/volatile/hallc/c-rsidis/outfiles/rsidis_H2_pos_inc_rad_kin%d.hist",runs[r]));
...
cout<<normfac_value<<endl;
	...
	TH1F *h_sdelta_simc = new TH1F("h_sdelta_simc","SHMS Delta;SHMS Delta;Entries",100,-12,22);
	TH1F *h_W_simc = new TH1F("h_W_simc","W;W;Entries",100,1.5,5);
	TH1F *h_hdelta_simc = new TH1F("h_hdelta_simc","HMS Delta;HMS Delta;Entries",100,-12,12);

	//TString filename  = Form("/lustre24/expphy/volatile/hallc/c-csv/hdbhatt/simcjuly22/worksim/csv_%d_H2_pos_inc_rad.root",runs[r]);
    TString filename  = Form("/lustre24/expphy/volatile/hallc/c-rsidis/worksim/rsidis_H2_pos_inc_rad_kin%d.root",runs[r]);
...
#include "includeRad.h"
	...
	h_hdelta_simc->Fill(hsdelta,normwt();
	h_pdelta_simc->Fill(ssdelta,normwt();
    ...
    std::string rootfile_yield_name = Form("yield_hists/yield_simc_inc_h2pos_test_%d.root",runs[r]);
    ...
    h_Q2_simc->SetTitle(Form("Q2 %d",runs[r]));
     ...
     c1->cd(4);
    h_pdelta_simc->Draw("hist");
    h_pdelta_simc->SetLineWidth(2);
    h_pdelta_simc->SetTitle(Form("SHMS Delta %d",runs[r]));

   c1->cd(4);
    h_hdelta_simc->Draw("hist");
    h_hdelta_simc->SetLineWidth(2);
    h_hdelta_simc->SetTitle(Form("HMS Delta %d",runs[r]));


    

    c1->SaveAs(Form("plots_simc/plots_h2pos_incrad_test_%d.pdf",runs[r]));
