...
void simc_yield_radd2pos(){
  ...
  ofstream txtsimcd2pos("textfile/yield_simc_D2pos.txt",ios::app);
  const int numRuns = sizeof(runs)/sizeof(*runs);
  for (int r =0;r<numRuns;r++){
  ifstream infile(Form("/lustre24/expphy/volatile/hallc/c-rsidis/outfiles/csv_%d_D2_pos_inc_rad.hist",runs[r]));
    //ifstream infile(Form("/lustre24/expphy/volatile/hallc/c-rsidis/outfiles/csv_%d_D2_pos_inc_rad.hist",runs[r]));
    if(!infile){
...
cout<<normfac_value<<endl;
    ...
    TString filename  = Form("/lustre24/expphy/volatile/hallc/c-rsidis/worksim/csv_%d_D2_pos_inc_rad.root",runs[r]);
    //TString filename  = Form("/lustre24/expphy/volatile/hallc/c-rsidis/worksim/csv_%d_D2_pos_inc_rad.root",runs[r]);
...
#include "includeRad.h"
    ...
    std::string rootfile_yield_name = Form("yield_hists/yield_simc_inc_d2pos_%d.root",runs[r]);
    ...
    txtsimcd2pos<<runs[r]<<"\t"<<yield_simc<<endl;
    ...
    c1->SaveAs(Form("plots_simc/plots_d2pos_incrad_%d.pdf",runs[r]));
